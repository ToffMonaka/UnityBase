 ...
 */
public class IniFileReadDescData : TextFileReadDescData
{
    /**
     * @brief コンストラクタ
     */
 ...
 */
public class IniFileWriteDescData : TextFileWriteDescData
{
    /**
     * @brief コンストラクタ
     */
...
{
	public IniFileData data = new();
	public FileReadDesc<IniFileReadDescData> readDesc = new();
	public FileWriteDesc<IniFileWriteDescData> writeDesc = new();
    ...
    protected override int _OnRead()
    {
	    ...
	    foreach (var txt_file_line_txt in txt_file.data.lineTextContainer) {
	        ...
	        {// コメントを削除
		        comment_str_index = line_txt.IndexOf(IniFileUtil.COMMENT_CODE);
	        ...
	        {// ｢=｣を確認
		        equal_str_index = line_txt.IndexOf(IniFileUtil.EQUAL_CODE);

		        if (equal_str_index < 0) {
			        section_start_str_index = line_txt.IndexOf(IniFileUtil.SECTION_START_CODE);

			        if (section_start_str_index < 0) {
				        continue;
			        }

			        section_end_str_index = line_txt.IndexOf(IniFileUtil.SECTION_END_CODE, section_start_str_index + IniFileUtil.SECTION_START_CODE.Length);

			        if (section_end_str_index < 0) {
				        continue;
			        }

			        string tmp_section_name = line_txt.Substring(section_start_str_index + IniFileUtil.SECTION_START_CODE.Length, section_end_str_index - (section_start_str_index + IniFileUtil.SECTION_START_CODE.Length));
	        ...
	        val = line_txt.Substring(equal_str_index + IniFileUtil.EQUAL_CODE.Length);
	        val = val.Trim();
    ...
    protected override int _OnWrite()
    {
        ...
        if (this.data.valueContainer.Count > 0) {
		    ...
		    foreach (var key_cont in this.data.valueContainer) {
			    line_txt = IniFileUtil.SECTION_START_CODE;
			    line_txt += key_cont.Key;
			    line_txt += IniFileUtil.SECTION_END_CODE;

			    txt_file.data.lineTextContainer.Add(line_txt);

			    foreach (var val in key_cont.Value) {
				    line_txt = val.Key;
				    line_txt += IniFileUtil.EQUAL_CODE;
				    line_txt += val.Value;
