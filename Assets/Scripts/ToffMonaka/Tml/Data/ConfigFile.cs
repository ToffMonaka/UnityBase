 ...
 */
public class ConfigFileReadDescData : TextFileReadDescData
{
    /**
     * @brief コンストラクタ
     */
 ...
 */
public class ConfigFileWriteDescData : TextFileWriteDescData
{
    /**
     * @brief コンストラクタ
     */
...
{
	public ConfigFileData data = new();
	public FileReadDesc<ConfigFileReadDescData> readDesc = new();
	public FileWriteDesc<ConfigFileWriteDescData> writeDesc = new();
    ...
    protected override int _OnRead()
    {
	    ...
	    foreach (var txt_file_line_txt in txt_file.data.lineTextContainer) {
	        ...
	        {// コメントを削除
		        comment_str_index = line_txt.IndexOf(ConfigFileUtil.COMMENT_CODE);
	        ...
	        {// ｢=｣を確認
		        equal_str_index = line_txt.IndexOf(ConfigFileUtil.EQUAL_CODE);
	        ...
	        val = line_txt.Substring(equal_str_index + ConfigFileUtil.EQUAL_CODE.Length);
	        val = val.Trim();
    ...
    protected override int _OnWrite()
    {
        ...
        if (this.data.valueContainer.Count > 0) {
	        ...
	        foreach (var val in this.data.valueContainer) {
		        line_txt = val.Key;
		        line_txt += ConfigFileUtil.EQUAL_CODE;
		        line_txt += val.Value;
