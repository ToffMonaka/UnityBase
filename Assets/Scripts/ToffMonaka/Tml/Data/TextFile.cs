...
{
	public TextFileData data = new();
	public FileReadDesc<TextFileReadDescData> readDesc = new();
	public FileWriteDesc<TextFileWriteDescData> writeDesc = new();
	    ...
	    if (desc_dat.filePath.Length <= 0) {
		    if (desc_dat.buffer.GetLength() <= 0) {
		        ...
		        if (desc_dat.text.Length > 0) {
                    this.data.lineTextContainer = new List<string>(desc_dat.text.Split(StringUtil.GetNewlineCode(desc_dat.newlineType)));
		        }
    ...
    protected override int _OnWrite()
    {
        ...
        string buf_str = System.String.Join(StringUtil.GetNewlineCode(desc_dat.newlineType), this.data.lineTextContainer);
        ...
        if (buf_str.Length > 0) {
	        ...
	        && (desc_dat.appendNewlineCount > 0)) {
		        ...
		        for (int newline_i = 0; newline_i < desc_dat.appendNewlineCount; ++newline_i) {
			        newline_code += StringUtil.GetNewlineCode(desc_dat.newlineType);
		        }
