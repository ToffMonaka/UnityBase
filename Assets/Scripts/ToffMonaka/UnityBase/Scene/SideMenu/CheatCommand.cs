...
public static class CheatCommandUtil
{
    ...
    public static readonly int ADD_CODE_TYPE_COUNT = (int)CheatCommandUtil.ADD_CODE_TYPE.COUNT;
    ...
    public static readonly CheatCommandUtil.ADD_CODE_TYPE[] FUNCTION_ADD_CODE_TYPE_ARRAY = {
        CheatCommandUtil.ADD_CODE_TYPE.FUNCTION_DELETE_DATA
    };
     ...
     * @return val (value)
     */
    public static double FunctionDeleteData(CheatCommand owner, double[] arg_val_ary, out int dst_result_val)
    {
...
public class CheatCommand
{
	...
	private CheatCommandCalculateOption _calculateOption;
	private bool _calculateTestFlag;
	private CheatCommandParser _parser;
     ...
     */
    public CheatCommandCalculateOption GetCalculateOption()
    {
     ...
     */
    public CheatCommandParser GetParser()
    {
        return (this._parser);
    }
     ...
     * 0未満=失敗
     */
    public int Calculate(out double dst_calc_val, CheatCommandCalculateOption calc_option, bool calc_test_flg = false, CheatCommandParser parser = null)
    {
        dst_calc_val = 0.0;

        this._calculateOption = calc_option;
        this._calculateTestFlag = calc_test_flg;
        this._parser = parser ?? new CheatCommandParser(this._code);
        ...
        foreach (var node in this._parser.GetNodeContainer()) {
            if (node.GetNodeType() == CheatCommandParserUtil.NODE_TYPE.EMPTY) {
                return (-1);
            }
     ...
     */
    public System.Tuple<int, double> _CalculateNode(CheatCommandParserNode node)
    {
        double calc_val = 0.0;

		switch (node.GetNodeType()) {
		case CheatCommandParserUtil.NODE_TYPE.NUMBER: {
		...
		case CheatCommandParserUtil.NODE_TYPE.UNARY_EXPRESSION: {
		...
		case CheatCommandParserUtil.NODE_TYPE.FUNCTION: {
