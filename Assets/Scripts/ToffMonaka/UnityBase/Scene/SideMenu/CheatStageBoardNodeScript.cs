 ...
 */
public class CheatStageBoardNodeScriptCreateDesc : StageBoardNodeScriptCreateDesc
{
}
 ...
 */
public class CheatStageBoardNodeScript : StageBoardNodeScript
{
    ...
    public new CheatStageBoardNodeScriptCreateDesc createDesc{get; private set;} = null;

    private CheatCommand _cheatCommand = new("");
    private CheatCommandCalculateOption _cheatCommandCalculateOption = new();
    private double _calculateValue = 0.0;
    private int _calculateResultValue = 0;
    private List<CheatStageBoardItemNodeScript> _itemNodeScriptContainer = new();
            ...
            foreach (var add_code_type in add_code_type_ary) {
                ...
                script_create_desc.onClick = (owner) =>
                {
                    if (this._cheatCommand.GetCode().Length <= 0) {
                        this.SetCheatCommand(CheatCommandUtil.ADD_CODE_TEXT_ARRAY[(int)owner.GetAddCodeType()] + ";");
                    } else {
                        this.SetCheatCommand(this._cheatCommand.GetCode() + " " + CheatCommandUtil.ADD_CODE_TEXT_ARRAY[(int)owner.GetAddCodeType()] + ";");
                    }
    ...
    {
        if (create_desc == null) {
            this.SetCreateDesc(new CheatStageBoardNodeScriptCreateDesc());

            return;
        }

	    this.createDesc = create_desc as CheatStageBoardNodeScriptCreateDesc;
