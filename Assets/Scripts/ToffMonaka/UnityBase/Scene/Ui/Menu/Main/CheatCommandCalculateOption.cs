...
{
    public delegate double Function(UnityBase.Scene.Ui.Menu.Main.CheatCommand owner, double[] arg_val_ary, out int dst_result_val);
    ...
    public void SetFunction()
    {
	    this._functionArray = new Function[]{
            new Function(UnityBase.Scene.Ui.Menu.Main.CheatCommandUtil.FunctionDeleteData)
        };

	    this._functionIndexContainer = new Dictionary<string, int>();

        for (int add_code_type_i = 0; add_code_type_i < UnityBase.Scene.Ui.Menu.Main.CheatCommandUtil.FUNCTION_ADD_CODE_TYPE_ARRAY.Length; ++add_code_type_i) {
        	this._functionIndexContainer[UnityBase.Scene.Ui.Menu.Main.CheatCommandUtil.ADD_CODE_NAME_ARRAY[(int)UnityBase.Scene.Ui.Menu.Main.CheatCommandUtil.FUNCTION_ADD_CODE_TYPE_ARRAY[add_code_type_i]]] = add_code_type_i;
        }
